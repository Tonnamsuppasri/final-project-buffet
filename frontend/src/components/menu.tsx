...
// ============================
const Menu = () => {
  // ... (Hooks and State declarations - คงเดิม) ...
  const location = useLocation();
  const navigate = useNavigate();

  const [sidebarOpen, setSidebarOpen] = useState(window.innerWidth >= 768);
  const [hoveredItem, setHoveredItem] = useState(null); // (ตัวแปรนี้ไม่ได้ใช้ แต่คงไว้เผื่อคุณใช้ในอนาคต)
...
  // ... (useEffect and Handlers - คงเดิมทั้งหมด) ...
  // useEffect (username check)
  // showToast (function)
  // useEffect (Socket and ShopLogo)
  // useEffect (Click Outside)
  // useEffect (Resize)
  // handleNavigation (function)
  // handleLogoClick (function)
  // handleOpenNotiCenter (function)
  // handleNotiClick (function)
...
          {/* ... (Enhanced Profile Section - คงเดิม) ... */}
...
            {/* --- 🛑 MOVED: ส่วนรายงานถูกย้ายจากตรงนี้ --- */}
...
                  </div>
                    ...
                    >
                      <div className="relative flex items-center justify-center mr-3 text-cyan-300 {/* Choose a color */}
                                      transition-all duration-300 group-hover/item:drop-shadow-lg">
                        {/* Use the imported icon */}
                        <ClipboardDocumentListIcon className={`w-6 h-6 sm:w-8 sm:h-8 transition-all duration-300
                                          ${isActive('/attendance-report') ? 'animate-pulse' : ''}
                                          group-hover/item:scale-110`} />
                      </div>
                      <span className="transition-all duration-300 group-hover/item:translate-x-1">
                        รายงานลงเวลา
                  ...
                  <div className="relative group/item">
                    ...
                    >
                      <div className="relative flex items-center justify-center mr-3 text-lime-300 transition-all duration-300 group-hover/item:drop-shadow-lg">
                        <PresentationChartLineIcon className={`w-6 h-6 sm:w-8 sm:h-8 transition-all duration-300
                                          ${isActive('/attendance-summary-report') ? 'animate-pulse' : ''}
                                          group-hover/item:scale-110`} />
                      </div>
                      <span className="transition-all duration-300 group-hover/item:translate-x-1">
                        สรุปเวลาทำงาน
...
            {/* (Main Menu) */}
            <nav className="space-y-2 w-full">
              {[
                ...
                // --- 👇 Add Stock Management Object Here 👇 ---
                { path: '/stock-management', label: 'จัดการสต็อก', icon: ArchiveBoxIcon, color: 'text-purple-300', adminOnly: true },
                // --- 👆 End Stock Management Object 👆 ---
              ].map((item) => {
                  // --- 👇 Add Role Check Here 👇 ---
                  // If adminOnly is true and the role is not 'Admin', return null (don't render)
                  ...
                  // --- End Existing Rendering Logic ---
              })}
            </nav>


            {/* --- ✅ MOVED: ย้ายส่วนรายงานมาไว้ที่นี่ --- */}
            {/* (Report Menu) */}
            {role === 'Admin' && (
              <>
                <div className="my-6 mx-4 relative">
...
                <nav className="space-y-2 w-full">
                  {/* ✅ MODIFIED: อัปเดตรายการรายงานสำหรับร้านชาบูบุฟเฟต์ */}
                  {[
                    { path: '/report-overview', label: 'ภาพรวม (Dashboard)', color: 'text-white' },
...
            backdrop-blur-sm z-20 transition-all duration-500" 
...
              bg-clip-text text-transparent animate-pulse">
              {/* ... (Dynamic Title - คงเดิม) ... */}
              {location.pathname === '/welcome' && 'หน้าหลัก'}
              {/* ✅ MODIFIED: เพิ่ม path ของรายงานใหม่ๆ */ }
              {location.pathname === '/report-overview' && 'ภาพรวมรายงาน'}
              {location.pathname === '/report-sales' && 'รายงานยอดขาย'}
              {location.pathname === '/report-menu' && 'รายงานเมนู/บุฟเฟต์'}
              {location.pathname === '/report-stock' && 'รายงานสต็อก'}
              {location.pathname === '/stock-management' && 'จัดการสต็อก'}
              {location.pathname === '/attendance-report' && 'รายงานลงเวลาทำงาน'}
...
          </h2>
          
          {/* ... (Notification Bell and Logout Button - คงเดิม) ... */}
          <div className="flex items-center gap-2 sm:gap-4 z-10">
            {/* ✨ === ADD CLOCK IN/OUT BUTTON HERE === ✨ */}
             <div className="flex items-center bg-white/40 p-2 rounded-lg shadow-inner"> {/* Optional styling container, hidden on smallest screens */}
                <ClockInOutButton />
             </div>
